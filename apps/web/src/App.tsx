...
type ComparisonHistorySyncWorkspaceSignalDetailNestedKey =
  | "laneSemantics"
  ...
  | "selectionAlignment"
  | "resolutionBasis";
type ComparisonHistorySyncWorkspaceSignalMicroViewKey =
  | "summary"
  | "trace"
  | "recommendation"
  | "alternative"
...
type ControlRoomComparisonHistoryPanelUiState = {
...
export default function App() {
  ...
  const [comparisonHistoryPanelOpen, setComparisonHistoryPanelOpen] = useState(
    () => initialComparisonHistoryPanelUiStateRef.current?.open ?? false,
  );
  const [comparisonHistorySearchQuery, setComparisonHistorySearchQuery] = useState(
    () => initialComparisonHistoryPanelUiStateRef.current?.searchQuery ?? "",
  );
  ...
  const [focusedWorkspaceScoreDetailSources, setFocusedWorkspaceScoreDetailSources] = useState<
  ...
  const [collapsedWorkspaceScoreSignalNestedSubviewIds, setCollapsedWorkspaceScoreSignalNestedSubviewIds] =
    useState<Record<string, boolean>>(
      () => initialComparisonHistoryPanelUiStateRef.current?.collapsedWorkspaceScoreSignalNestedSubviewIds ?? {},
    );
  const [focusedWorkspaceScoreSignalMicroViews, setFocusedWorkspaceScoreSignalMicroViews] = useState<
  ...
  const latestWorkspaceSyncState = useMemo(
    () => buildComparisonHistorySyncWorkspaceState({
      comparisonSelection,
      expandedGapRows,
      expandedGapWindowSelections,
    }),
    [comparisonSelection, expandedGapRows, expandedGapWindowSelections],
  );
  ...
  useEffect(() => {
    ...
    const handlePopState = (event: PopStateEvent) => {
      ...
      if (historyBrowserState) {
        ...
        setComparisonHistoryPanel((current) => {
          const matchingEntry = findComparisonHistoryPanelEntryForSelection(current, nextSelection);
          return matchingEntry
            ? {
                ...current,
                activeEntryId: matchingEntry.entryId,
              }
            : current;
        });
      }
  ...
  useEffect(() => {
    ...
    persistControlRoomUiState({
      comparisonSelection,
      comparisonHistoryPanel: {
        open: comparisonHistoryPanelOpen,
  ...
  useEffect(() => {
    ...
    const matchingPersistedEntry =
      currentBrowserState
        ? null
        : findComparisonHistoryPanelEntryForSelection(comparisonHistoryPanel, comparisonSelection);
    const nextEntryId =
      comparisonHistoryWriteModeRef.current === "push"
        ? buildComparisonHistoryEntryId()
        : currentBrowserState?.entryId
          ?? matchingPersistedEntry?.entryId
          ?? buildComparisonHistoryEntryId();
    const nextStepIndex =
      comparisonHistoryWriteModeRef.current === "push"
        ? (
            currentBrowserState?.stepIndex
            ?? matchingPersistedEntry?.stepIndex
  ...
  }, [comparisonHistoryActiveStepIndex, comparisonHistoryPanel, comparisonHistoryStep, comparisonSelection]);
  ...
  const handleApplyComparisonHistoryWorkspaceResolution = (auditId: string) => {
    ...
    const resolvedState = resolveComparisonHistorySyncWorkspaceReview(
