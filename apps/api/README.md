...
- guarded-live incident delivery now supports console, generic webhook, Slack webhook, PagerDuty
  Events API, incident.io, FireHydrant, Rootly, Blameless, xMatters, ServiceNow, and Opsgenie Alert API targets with persisted attempt history, operator
...
- full external incident-management workflow such as provider-managed incident ownership beyond the
  current PagerDuty/incident.io/FireHydrant/Rootly/Blameless/xMatters/ServiceNow/Opsgenie-native bidirectional paths, richer escalation ladders, and broader
...
- guarded-live alert transitions now emit durable incident-opened/resolved events and outbound
  ...
  FireHydrant delivery, Rootly delivery, Blameless delivery, xMatters delivery, ServiceNow delivery, or Opsgenie Alert API
...
- external paging systems can now push `triggered`, `acknowledged`, `escalated`, `resolved`, and
  ...
  count. Guarded-live state refresh now also pull-syncs the current PagerDuty/incident.io/FireHydrant/Rootly/Blameless/xMatters/ServiceNow/Opsgenie
  ...
  provider-specific schemas for PagerDuty incidents, incident.io incidents, FireHydrant incidents, Rootly incidents, Blameless incidents, xMatters incidents, ServiceNow incidents, and Opsgenie alerts instead of
  ...
  across PagerDuty, incident.io, FireHydrant, Rootly, Blameless, xMatters, ServiceNow, and Opsgenie
