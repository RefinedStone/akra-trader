...
class OperatorDeliverySharedHelpersMixin:
  ...
  def _format_recovery_engine_url(
    *,
    ...
    if not url_template:
      return None
    context = OperatorDeliverySharedHelpersMixin._build_recovery_engine_template_context(
      workflow_reference=workflow_reference,
      external_reference=external_reference,
      job_id=job_id,
    )
  ...
  def _build_provider_recovery_payload(incident: OperatorIncidentEvent) -> dict[str, Any]:
    ...
    market_context = OperatorDeliverySharedHelpersMixin._build_incident_market_context_payload(incident)
    return {
    ...
    return json.dumps(
      {
        ...
        **market_context,
        "remediation": {
          ...
          "provider_recovery": OperatorDeliverySharedHelpersMixin._build_provider_recovery_payload(incident),
        },
      }
    ).encode("utf-8")
  @staticmethod
      ...
      data=json.dumps(
        {
          "incident": {
            ...
            "metadata": {
              ...
              "remediation_provider_recovery": OperatorDeliverySharedHelpersMixin._build_provider_recovery_payload(incident),
            },
          }
        }
      ).encode("utf-8"),
