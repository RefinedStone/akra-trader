...
def _normalize_target(target: str) -> str | None:
  ...
  if normalized in {
    "crisescontrol",
    "crises_control",
    "crisescontrol_incidents",
    "crisescontrol_alerts",
    "operator_crisescontrol",
  }:
    return "crisescontrol_incidents"
  if normalized in {
    "freshservice",
    "fresh_service",
    "freshservice_incidents",
    "freshservice_alerts",
  ...
  if normalized in {
    ...
    return "bmchelix_incidents"
  if normalized in {
    "solarwindsservicedesk",
    "solarwinds_service_desk",
    "solarwindsservicedesk_incidents",
    "solarwindsservicedesk_alerts",
    "operator_solarwindsservicedesk",
  }:
    return "solarwindsservicedesk_incidents"
  if normalized in {"topdesk", "topdesk_incidents", "topdesk_alerts", "operator_topdesk"}:
    return "topdesk_incidents"
...
class OperatorAlertDeliveryAdapter(OperatorAlertDeliveryPort):
  ...
  def list_supported_workflow_providers(self) -> tuple[str, ...]:
    ...
    if self._crisescontrol_api_token:
    ...
    if self._sysaid_api_token:
      providers.append("sysaid")
    if self._bmchelix_api_token:
      providers.append("bmchelix")
    if self._solarwindsservicedesk_api_token:
      providers.append("solarwindsservicedesk")
    if self._topdesk_api_token:
      providers.append("topdesk")
    ...
    *,
    ...
    for target in resolved_targets:
      ...
      if target == "squzy_incidents":
        ...
        continue
      if target == "crisescontrol_incidents":
        records.append(
          self._deliver_crisescontrol(
            incident=incident,
            attempt_number=attempt_number,
            phase=phase,
          )
      ...
      if target == "servicedeskplus_incidents":
        ...
        continue
      if target == "sysaid_incidents":
        records.append(
          self._deliver_sysaid(
            incident=incident,
            attempt_number=attempt_number,
            phase=phase,
          )
        ...
        continue
      if target == "bmchelix_incidents":
        records.append(
          self._deliver_bmchelix(
            incident=incident,
            attempt_number=attempt_number,
            phase=phase,
          )
        ...
        continue
      if target == "solarwindsservicedesk_incidents":
        records.append(
          self._deliver_solarwindsservicedesk(
            incident=incident,
            attempt_number=attempt_number,
            phase=phase,
          )
        ...
        continue
      if target == "topdesk_incidents":
        records.append(
          self._deliver_topdesk(
            incident=incident,
            attempt_number=attempt_number,
            phase=phase,
          )
  ...
  def sync_incident_workflow(
    ...
    *,
    ...
    if normalized_provider == "crisescontrol":
      return (
        self._sync_crisescontrol_workflow(
          incident=incident,
          action=normalized_action,
          actor=actor,
          detail=detail,
    ...
    if normalized_provider == "freshservice":
      return (
        self._sync_freshservice_workflow(
          incident=incident,
          action=normalized_action,
          actor=actor,
          detail=detail,
    ...
    if normalized_provider == "bmchelix":
      return (
        self._sync_bmchelix_workflow(
          incident=incident,
          action=normalized_action,
          actor=actor,
          detail=detail,
    ...
    if normalized_provider == "solarwindsservicedesk":
      return (
        self._sync_solarwindsservicedesk_workflow(
          incident=incident,
          action=normalized_action,
          actor=actor,
          detail=detail,
    ...
    if normalized_provider == "topdesk":
      return (
        self._sync_topdesk_workflow(
          incident=incident,
          action=normalized_action,
          actor=actor,
          detail=detail,
  ...
  def pull_incident_workflow_state(
    ...
    *,
    ...
    if normalized_provider == "sysaid":
      return self._pull_sysaid_workflow_state(incident=incident)
    if normalized_provider == "bmchelix":
      return self._pull_bmchelix_workflow_state(incident=incident)
    if normalized_provider == "solarwindsservicedesk":
      return self._pull_solarwindsservicedesk_workflow_state(incident=incident)
    if normalized_provider == "topdesk":
      return self._pull_topdesk_workflow_state(incident=incident)
    if normalized_provider == "opsramp":
