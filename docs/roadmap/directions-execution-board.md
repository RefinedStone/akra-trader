...
25. keep stitched report approval queue-state ownership balanced between summary, filter-state, and
    async-state collaborators instead of pushing queue async readout down into the filter ownership
    layer
26. keep stitched report approval queue filter-state ownership directly above query-input and
