...
Over the next 6 months, `akra-trader` should evolve from an architectural skeleton into a usable single-operator trading research platform for crypto markets, with guarded progression from research to paper trading and then to controlled live execution.

The primary user is a solo developer or quant who needs:

- reproducible backtests
- fast strategy iteration
- an operations view for running paper and live sessions
...
5. monitor a paper run in near real time
...
## Stage 3: Real-Time Paper Operations
...
- a user can keep paper strategies running continuously and inspect operational health from one dashboard

Required outcomes:

- continuous market-data stream or polling worker
- long-running paper execution worker
...
- paper runs can stay active without manual replay triggering
...
- LLM strategies can be evaluated in backtest and paper modes through the same decision envelope
...
- at least one exchange is supported end-to-end for data, paper, and guarded live execution
