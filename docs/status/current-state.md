...
- Binance push-session supervision now performs automatic listen-key failover and records broader
  account-position, balance-update, order-list, trade-tick, book-ticker, depth, and kline
  coverage alongside execution events
...
- guarded-live reconciliation and runtime recovery now depend on configured venue credentials, and
  ...
  session coverage beyond Binance multi-stream account/order/trade/book-ticker/depth/kline events
...
- broader venue-native session continuation beyond Binance multi-stream
  account/order/trade/book-ticker/depth/kline streaming
