...
- provider-provenance export, scheduler core, scheduler search, analytics dashboard,
...
- the scheduler narrative registry table collaborator now composes dedicated table-view,
  row-detail, and row-action collaborators instead of mixing table shell, row review, and row
  actions inline in one registry module
...
- the stitched report approval queue queue-state, lane, and priority selector collaborators now
  compose dedicated option-source collaborators instead of calculating selector option sources
  inline in each selector leaf
