...
faults, exchange-specific ladder bridge/sequence/snapshot-refresh faults, deeper depth-ladder/
candle-sequence semantics, and multi-candle continuity faults into the same durable incident
surface. Alert transitions now emit durable incident-opened/resolved events plus outbound delivery
