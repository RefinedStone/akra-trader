...
top-of-book levels. When Binance user-data streaming is unavailable, the guarded-live adapter can
  ...
  FireHydrant, Rootly, Blameless, xMatters, ServiceNow, Squadcast, BigPanda, Grafana OnCall, and Opsgenie Alert API targets, while the application layer owns acknowledgment, escalation timing, retry suppression, and
  ...
  refreshes. When configured, PagerDuty, incident.io, FireHydrant, Rootly, Blameless, xMatters, ServiceNow, Squadcast, BigPanda, Grafana OnCall, and Opsgenie remediation-engine endpoints are also polled
